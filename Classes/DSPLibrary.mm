...
// design parametric filter based on input center frequency, gain, Q and sampling rate
{
    ...
    if (gain > 0.0) { // boost case
        b0 = 1;
        b1 = gain / (qval * center * 2 * PI);
        b2 = 1 / (center * center * 4 * PI * PI);
        a0 = 1;
        a1 = 1 / (qval * center * 2 * PI);
        a2 = 1 / (center * center * 4 * PI * PI);
    } else { // cut case
        b0 = 1;
        b1 = 1 / (qval * center * 2 * PI);
        b2 = 1 / (center * center * 4 * PI * PI);
        a0 = 1;
        a1 = 1 / (qval * gain * center * 2 * PI);
        a2 = 1 / (center * center * 4 * PI * PI);
    }
	...
	bilinearTranform(acoefs, dcoefs, withFs); // inputs the 6 analog coeffs, output the 5 digital coeffs
}
...
// design parametric filter based on input center frequency, gain, Q and sampling rate
{
	...
	// a2s  + a1s + a0
	//
    // where b2 and a2 are zero (1st order, just keePIng them so the same BLT function can be used)
    //
